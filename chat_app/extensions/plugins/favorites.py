...
from chat_app.data.favorites_store import FAVORITES_AUDIO_DIR, FavoritesStore
...
class FavoritesExtension(BaseExtension):
    ...
    def on_stop(self) -> None:
        ...
        if self._synth_thread is not None and self._synth_thread.isRunning():
            self._synth_thread.stop()
            if not self._synth_thread.wait(2000):
                self._synth_thread.terminate()
